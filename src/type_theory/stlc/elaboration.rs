...
pub fn elaborate_abstraction(
    ...
    let (var_type_term, _) = Stlc::elaborate_expression(var_type, environment);
    ...
    let (body_term, body_type) = Stlc::elaborate_expression(body, environment);
...
pub fn elaborate_application(
    ...
    let (left_term, left_type) = Stlc::elaborate_expression(left, environment);
    let (right_term, _) = Stlc::elaborate_expression(right, environment);
...
pub fn elaborate_let(
    ...
    let (assigned_term, term_type) =
        Stlc::elaborate_expression(ast, environment);
...
pub fn elaborate_file_root(
    ...
    for sub_ast in asts {
        match sub_ast {
            ...
            NsAst::Exp(exp) => {
                let _ = Stlc::elaborate_expression(exp, environment);
            }
        }
    }
}
